...
var voteskipCommand = function message) {
    if (message.guild) {
        voteSkipmessage, function (err, response) {
            if (err) {
...
var voteSkip = function voteSkipmessage, cb) {
            ...
            if (Queue) {
                if (Queue.songs.length > 0) {
                    if (voice.inVoicemessage)) {
                        var channel = voice.getVoiceChannelmessage);
                                                ...
                                                if (Queue) {
                                                    ...
                                                    if (votePercentage > 0.51) {
                                                        Queue.stopRepeat(function (err) {
                                                            if (err) return cb(err);
                                                            voice.nextSongmessage, Queue.songs[0]);
